  ...
  <section v-loading="loading" class="edit_mang_wrap">
    <div class="mang_inner comp_inner">
      <div class="mang_title">
        <h3 class="mb-3">
          {{ router.i_details ? "Edit" : "" }} Insurance details
        </h3>
            ...
            <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
              <div class="fieldrow">
                <el-form-item
                  ...
                  prop="name_of_insurence_company"
                >
                  <el-input
                    v-model="formData.name_of_insurence_company"
            ...
            <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
              <div class="fieldrow">
                <el-form-item
                  ...
                  prop="expiry_of_insurence"
                >
                  <el-date-picker
                    value-format="YYYY-MM-DD"
                    v-model="formData.expiry_of_insurence"
                    type="date"
...
import { postCompanyInsurenceDetails } from "@/api/distributor/add-distributor";
      ...
      if (valid && !isUploaded.value) {
        try {
          ...
          const insurance_detail = {
            ...
            insurance_company: formData.name_of_insurence_company,
            expiry: formData.expiry_of_insurence,
            is_verified: true,
          };
          postCompanyInsurenceDetails(insurance_detail, companyId.value)
            .then((res) => {
              if (res.status === 201) {
                sessionStorage.setItem("insurance_id", res.data.id);
                axios
                  .put(res.data.insurance_copy, formData.insurence_card)
                  .then((res) => {
                    if (res.status === 200) {
                      flashNotification(
                        "success",
                        `Successfully uploaded insurance details`
                      );
...
//here handleAssignDepot function
// function handleAssignDepot() {
//   if (router.i_details) {
//     delete router.i_details;
//     router.push(`/distributor/distributor-${"undefined"}`);
//   } else {
//     router.push("/distributor/add-distributor/depot-assign");
//   }
// }
...
//onmounted
onMounted(() => {
  navStore.setBreadCrumbs("Distributor / Add insurance");
  companyId.value = sessionStorage.getItem("company_id");
  if (router.b_details) {
    Object.assign(formData, router.i_details);
    navStore.setBreadCrumbs("Distributor / Edit insurance");
  }
});
