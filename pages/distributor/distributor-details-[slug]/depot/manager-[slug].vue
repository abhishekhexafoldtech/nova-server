  ...
  <div class="container-area">
    <div class="cd_header">
      <div class="header_inner">
        <img
          :src="user.header_details.profile_pic"
          alt="Profile"
        ...
        <div class="h_right">
          ...
          <div class="h_info">
            ...
            <div>
              <p class="text-capitalize">Depot name:<span >{{ user.header_details.depot_name || "-"}}</span></p>
...
<script setup>
import { ref } from "vue";
import PersonalDetailes from "@/components/distributor/location/depot-manager/PersonalDetailes";
...
async function getDepotManagerDetails() {
  const routeParams = `tab=personal_details&employee_type=depot_manager`
  try {
    // const data = await getEmployee(managerId.value,routeParams);
    const data = await getEmployee("39ffe58e-46ae-4956-8261-8548f512b356", routeParams);
    user['personal_details'] = await data.data[0];
    setHeaderDetails();
  } catch (err) {
...
async function setHeaderDetails() {
  ...
  const role = header_details.role.length ? header_details?.role.split("_").join(" ") : "-";
  const depot_name = user[activeName.value].personal_details?.address[0]?.name;
  user.header_details = { ...header_details, role, depot_name }
...
onMounted(async()=>{
  ...
  managerId.value = path[path.length - 1].split("-").pop();
  await getDepotManagerDetails();
})
