                      ...
                      >
                        ...
                        <span v-if="config.popover">
                          <span>
                            <el-link
                              type="primary"
                              @click="handleView(scope.row,scope.column.property)"
                              :underline="false"
                            >
                              <span>{{
                        ...
                        <span v-if="item.status === true" class="name_status"
                          ><i class="ri-checkbox-circle-line"></i
                        ></span>
                        <span
                          v-if="item.status === false"
                          class="name_status pending"
                  ...
                  >
                    <span v-if="scope.row[config.prop][0].status">
                      <span
                        ...
                        :key="index"
                      >
                        <span v-if="item.status.toString().toLowerCase() == 'delivered' || item.date">
                        ...
                        <span v-if="item.status.toString().toLowerCase() == 'approve'">
                          <span class="text-success">{{
                            item ? item.status : ""
                          }}</span>
                        </span>

                        <span v-if="item.status.toString().toLowerCase() == 'shipped'">
                          <span class="text-primary">{{
                            item ? item.status : ""
                          }}</span>
                        </span>

                        <span v-if="item.status.toString().toLowerCase() == 'resolve'">
                          <span class="text-primary">{{
                            item ? item.status : ""
                          }}</span>
                        </span>

                        <span v-if="item.status.toString().toLowerCase() == 'pending'">
          ...
          <el-table-column v-if="actionVisibility" width="150" label="Actions">
            <template #default="scope">
              ...
              <span
                ...
                v-if="editButtonVisibility && !scope?.row?.child"
              ...
              <span
                ...
                v-if="deleteButtonVisibility && !scope?.row?.child"
              ...
              >
              <!-- more option 3 dot -->
                <span class="el-dropdown-link px-2">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item
                      v-if="viewStartPhyicalVarificationVisibility"
                      @click="handleViewStartPhyicalVarification(scope.row)"
                      >Start physical verification</el-dropdown-item
                    >

                    <el-dropdown-item
                      v-if="viewVerificationFormVisibility"
                      @click="handleViewVerificationForm(scope.row)"
                      >Verification form</el-dropdown-item
                    >
...
let props = defineProps({
  ...
  viewStartPhyicalVarificationVisibility: {
    type: Boolean,
    default: false
  },
  viewVerificationFormVisibility: {
    type: Boolean,
    default: false
  },
...
function handleViewStartPhyicalVarification(row) {
  emit("startPhyicalVarification", row);
}

function handleViewVerificationForm(row) {
  emit("viewVerificationForm", row)
}

function handleOptionClick(data,item){
 emit("optionClick",data,item );
}
...
function handleView(data,c_name) {
  emit("view", data,c_name);
}
